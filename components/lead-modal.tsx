...
export function LeadModal() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setCurrentStep(1) // Reset step counter on open
    } else {
  ...
  const isSchedulingStep = currentStep === 8
  ...
  return (
    ...
    >
      {/* Modal Card */}
      <div
        className={`relative w-full bg-surface border border-outline-variant/60 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row transition-all duration-500 ${
          isSchedulingStep
            ? 'max-w-5xl h-[680px] md:h-[650px]'
            : 'max-w-4xl h-[620px] md:h-[580px]'
      ...
      >
        ...
        {/* Left Side: Premium Dark Value Panel (Desktop Only, hidden on scheduling step) */}
        {!isSchedulingStep && (
        ...
        <div className={`p-6 sm:p-8 md:p-10 flex flex-col justify-between h-full bg-white select-text overflow-y-auto transition-all duration-500 ${
          isSchedulingStep ? 'w-full' : 'w-full md:w-7/12'
        }`}>
          <SparkLeadWizard theme="light" onClose={closeModal} onStepChange={setCurrentStep} />
        </div>
      </div>
    </div>
  )
